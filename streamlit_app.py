...
RAW=ROOT/"data/raw/Dispute_Head_UPE_Indus_jan23toDec23.xlsx"
...
def get_secret():
    try:
        ...
        if key: os.environ["GEMINI_API_KEY"]=str(key)
    except Exception: pass

def load_tax(upload=None):
    if upload is not None: return prepare_taxonomy(read_upload(upload))
    if TAX_XLSX.exists(): return prepare_taxonomy(read_path(TAX_XLSX))
...
def main():
    ...
    with st.sidebar:
        ...
        if source=="Upload files":
            uploaded=st.file_uploader("Billing/dispute file",type=["xlsx","xls","csv"])
            training_upload=st.file_uploader("Optional separate labelled training file",type=["xlsx","xls","csv"])
            tax_upload=st.file_uploader("Optional dispute reasons master",type=["xlsx","xls","csv"])
            kpi_upload=st.file_uploader("Optional network KPI file",type=["xlsx","xls","csv"])

    if source=="Upload files" and uploaded is None:
        st.info("Upload a billing file, or switch to Synthetic demo."); return

    try:
        training=None
        if source=="Synthetic demo":
            raw=demo_raw(); network=demo_kpis(); mode="raw"
        elif source=="Repository data":
            network=read_path(KPI_XLSX) if KPI_XLSX.exists() else None
            if repo_file=="Raw dispute history":
                raw=read_path(RAW); mode="raw"
            else:
                raw=read_path(FILTERED); mode="model-ready"
                if RAW.exists(): training=prepare(read_path(RAW),"raw",network).analytical
        else:
            raw=read_upload(uploaded); network=read_upload(kpi_upload) if kpi_upload else None; mode="auto"
            if training_upload: training=prepare(read_upload(training_upload),"auto",network).analytical
        taxonomy=load_tax(tax_upload)
        p=prepare(raw,mode,network); df=p.analytical
        ...
        st.error(f"Pipeline failed: {exc}"); return

    c1,c2,c3,c4=st.columns(4)
    c1.metric("Rows",f"{len(analyzed):,}"); c2.metric("Sites",analyzed["IP Site ID"].nunique())
    ...
    with tabs[0]:
        ...
        if not p.rejected.empty: st.warning(f"Rejected raw rows: {len(p.rejected)}")
    ...
    with tabs[1]:
        ...
        st.plotly_chart(disputes(analyzed),use_container_width=True); st.dataframe(analyzed[cols],use_container_width=True)
    ...
    with tabs[3]:
        if "Dispute Type" in df and df["Dispute Type"].notna().any():
            if st.button("Run chronological evaluation"):
                metrics,pred=evaluate_time_split(df,taxonomy,conf); st.json(metrics); st.dataframe(pred.head(100),use_container_width=True)
        else: st.info("Actual Dispute Type labels are required for evaluation.")
    with tabs[4]:
        ...
        if st.button("Generate grounded RCA"):
            with st.spinner("Generating RCA..."): st.json(rca_for_row(analyzed.loc[idx],taxonomy,model_name))
    with tabs[5]:
        q=st.text_input("Ask about a site, month, EB/DG dispute or anomaly")
        if st.button("Ask") and q:
            with st.spinner("Retrieving relevant structured rows..."): st.write(answer_query(q,analyzed,model_name))

    st.divider()
    st.download_button("Download complete analysis workbook",excel_bytes({"Analysis":analyzed,"Rejected Rows":p.rejected}),"vi_energy_rca_analysis.xlsx","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
